...
DataAccessObject.create = function (data, callback) {
  ...
  Model.notify('before save', { Model: Model, instance: obj }, function(err) {
        ...
        this._adapter().create(modelName, this.constructor._forDB(obj.toObject(true)), function (err, id, rev) {
          ...
          saveDone.call(obj, function () {
            createDone.call(obj, function () {
              ...
              Model.notify('after save', { Model: Model, instance: obj }, function(err) {
                callback(err, obj);
...
DataAccessObject.updateOrCreate = DataAccessObject.upsert = function upsert(data, callback) {
  ...
  Model.notify('query', { Model: Model, query: byIdQuery(Model, id) }, doUpdateOrCreate);

  function doUpdateOrCreate(err, ctx) {
    if (err) return callback(err);

    if (Model.getDataSource().connector.updateOrCreate && isWhereByGivenId(Model, ctx.query.where, id)) {
      var update = data;
      var inst = data;
      ...
      Model.notify('before save', { Model: Model, instance: inst }, function(err) {
        if (err) return callback(err);
      ...
      Model.findOne({ where: ctx.query.where }, function (err, inst) {
        if (err) {
          return callback(err);
        }
        if (inst) {
...
DataAccessObject.find = function find(query, cb) {
  ...
  var self = this;
  this.notify('query', { Model: this, query: query }, function(err, ctx) {
...
DataAccessObject.remove = DataAccessObject.deleteAll = DataAccessObject.destroyAll = function destroyAll(where, options, cb) {
  ...
  function doDelete(where) {
    ...
    function done(err, data) {
      ...
      Model.notify('after delete', { Model: Model, where: where }, function(err) {
        cb(err, data);
        if (!err)
...
DataAccessObject.count = function (where, cb) {
  ...
  var Model = this;
  this.notify('query', { Model: Model, query: { where: where } }, function(err, ctx) {
      if (err) return cb(err);
      ...
      inst.trigger('save', function (saveDone) {
        inst.trigger('update', function (updateDone) {
          ...
          inst._adapter().save(modelName, inst.constructor._forDB(data), function (err) {
            ...
            Model.notify('after save', { Model: Model, instance: inst }, function(err) {
...
DataAccessObject.prototype.updateAttributes = function updateAttributes(data, cb) {
  ...
  Model.notify('before save', context, function(err, ctx) {
    if (err) return cb(err);
    data = ctx.data;
